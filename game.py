...
tamanho_tela = width, heigth = (700,650)
estrada = int(width/1.6)
linhas_estrada = int(width/80)
faixa_direita = int(width/2 + estrada/4)
faixa_esquerda = int(width/2 - estrada/4)
velocidade= 1
...
carro_1_loc.center = faixa_direita,heigth*0.8

carro_2 = pygame.image.load("carro_2.png")
carro_2_loc = carro_2.get_rect()
carro_2_loc.center = faixa_esquerda,heigth*0.2

game_over =  pygame.image.load("game_over.png")
game_over_loc= game_over.get_rect()
game_over_loc.center = width/2,heigth*0.5
...
while corrida:
    ...
    if carro_1_loc[0] == carro_2_loc[0] and carro_2_loc[1] > carro_1_loc[1] -250:
         print("GAME OVEER! Voçê perdeu")
         screen.blit(game_over,game_over_loc)
         pygame.display.update()
         time.sleep(3)
         break
    
    if carro_2_loc[1] > heigth:
    ...
    pygame.draw.rect(
        screen,
        (50,50,50),
        (width/2-estrada/2,0,estrada,heigth))
    
    pygame.draw.rect(
        screen,
        (255,240,60),
        (width/2-linhas_estrada/2,0,linhas_estrada,heigth))
    
    pygame.draw.rect(
        screen,
        (255,255,255),
        (width/2-estrada/2 + linhas_estrada*2,0,linhas_estrada,heigth))
    
    pygame.draw.rect(
        screen,
        (255,255,255),
        (width/2+ estrada/2 - linhas_estrada*3,0,linhas_estrada,heigth))
